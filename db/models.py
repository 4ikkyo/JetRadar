...
class Wallet(Base):
    ...
    last_balance_ton = Column(DECIMAL(precision=20, scale=9),
                              nullable=True)  # Хранение баланса в nanoTON как Integer или TON как Decimal
...
class Transaction(Base):  # Новая таблица для локального хранения транзакций
    ...
    id = Column(Integer, primary_key=True, index=True)
    wallet_id = Column(Integer, ForeignKey('wallets.id'), nullable=False,
                       index=True)  # К какому из НАШИХ отслеживаемых кошельков относится
