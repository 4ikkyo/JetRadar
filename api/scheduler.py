...
async def fetch_and_store_wallet_activity(wallet_id: int, wallet_address: str):
    ...
    try:
        ...
        async with async_session() as session:
            async with session.begin():
                ...
                wallet_update_values = {
                    "last_balance_updated_at": datetime.utcnow()}  # Время последней успешной проверки
                if latest_event_ts:
                ...
                await session.execute(
                    update(Wallet).where(Wallet.id == wallet_id).values(**wallet_update_values)
                )
        ...
        logger.error(f"Непредвиденная ошибка при обновлении {wallet_address} (ID: {wallet_id}): {str(e)}",
                     exc_info=True)
...
async def scheduled_wallet_updates():
    ...
    for wallet_id, wallet_address in wallets_to_update:
        try:
            ...
            logger.error(f"Ошибка при обработке кошелька {wallet_address} в scheduled_wallet_updates: {e}",
                         exc_info=True)
