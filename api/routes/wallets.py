...
async def get_wallet_summary(address: str, telegram_user_id: int = Query(...)):
    ...
    async with async_session() as session:
        ...
        stmt = (
            ...
            .outerjoin(
                UserWallet,
                (UserWallet.wallet_id == Wallet.id) & (UserWallet.user_id == user.id)
            )
...
async def search_user_wallets(telegram_user_id: int = Query(...), query: str = Query(..., min_length=3)):
    ...
    async with async_session() as session:
        ...
        return [
            {"address": r.address, "alias": r.alias, "group": r.group}
            for r in found_wallets
        ]
...
async def get_connection_graph_api(telegram_user_id: int = Query(...),
    ...
    if not final_nodes and not edges_list:
        return GraphResponse(nodes=[], edges=[],
                             message="Не удалось найти транзакции для построения графа по заданным параметрам.")
...
async def add_wallet_to_user_watchlist(item: AddWalletRequest):
    ...
    Создает пользователя и/или кошелек, если они не существуют.
    """
    try:
        async with async_session() as session:
            # Используем session.begin() для автоматического commit/rollback
            async with session.begin():
                # Найти или создать пользователя
                user = await session.scalar(select(User).where(User.telegram_id == item.telegram_user_id))
                if not user:
                    logger.info(f"Создание нового пользователя: telegram_id={item.telegram_user_id}, username='{item.username}'")
                    user = User(telegram_id=item.telegram_user_id, username=item.username or f"user_{item.telegram_user_id}")
                    session.add(user)
                    await session.flush() # Чтобы получить user.id для дальнейшего использования
                ...
                if not wallet:
                    ...
                    session.add(wallet)
                    await session.flush() # Чтобы получить wallet.id
                ...
                if link:
                    logger.warning(f"Попытка повторного добавления: Кошелек {item.address} уже в Watchlist для пользователя {item.telegram_user_id}")
                    # Важно! HTTPException нужно вызывать вне session.begin(), если хотим откатить транзакцию
                    # Но в данном случае, если ссылка уже есть, то это не ошибка БД, а логическая ошибка запроса.
                    # Мы можем либо оставаться в session.begin() и ничего не менять, либо вынести HTTPException.
                    # Если выносим, то нужно будет убрать session.begin() или изменить логику.
                    # Для простоты оставим так, но это означает, что транзакция может быть закоммичена,
                    # даже если пользователь или кошелек были созданы, но связь уже существовала.
                    # Более корректно было бы сделать проверки до session.begin() или разделить логику.
                    # Пока оставим так, так как предыдущие flush уже произошли бы.
                    raise HTTPException(
                ...
                session.add(uw)
                await session.flush() # Чтобы uw.id был доступен перед возвратом

            # После успешного session.begin() (т.е. commit)
            logger.info(f"Успешно добавлен кошелек {wallet.address} (alias: '{uw.alias}') для пользователя {user.telegram_id}")
            return {
...
async def update_wallet_label_api(address: str, item: UpdateWalletLabelRequest):
    ...
    async with async_session() as session:
        ...
        user_wallet = await session.scalar(
            select(UserWallet)
            .where(UserWallet.user_id == user.id)
            .where(UserWallet.wallet_id == wallet.id)
        )

        if not user_wallet:
            raise HTTPException(status_code=404, detail=f"Кошелек {address} не найден в Watchlist пользователя {item.telegram_user_id}")
        ...
        for action in event.get("actions", []):
            ...
            if action_type == "TonTransfer":
                ...
                action_details.update({
                    ...
                    "jetton_image": jetton_info.get("metadata", {}).get("image") or jetton_info.get("metadata", {}).get(
                        "logo"),
                    "is_send": (sender_addr == address) if sender_addr else False
                })
            ...
            # --- Обработка Swap (пример) ---
                ...
                asset_out_symbol, amount_out, jetton_out_addr, jetton_out_img = parse_asset(asset_out_info, amount_out_str)
                ...
                jetton_image = jm_out.get("image") or jm_out.get("metadata", {}).get("image") or jm_out.get("metadata",
                                                                                                            {}).get(
                    "logo")
