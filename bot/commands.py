...
async def add_wallet_via_api(message: Message):
    ...
    if len(args) < 2:
        await message.answer(
            "❗ Будь ласка, вкажіть адресу гаманця: <code>/add &lt;TON-адреса&gt; [Метка]</code>", parse_mode="HTML"
        )
        return
        ...
        try:
            ...
            if response.status_code == 201:  # Created
                await message.answer(
                    ...
                    + (
                        f" з меткою \"{response_data.get('alias', alias)}\""
                        if response_data.get("alias", alias)
                        else ""
                    )
                    + " додано до твого Watchlist.",
                    parse_mode="HTML",
                )
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 200:
                ...
                if not wallets_data:
                    await message.answer(
                        "ℹ️ Твій Watchlist порожній. Додай гаманець командою <code>/add</code>.", parse_mode="HTML"
                    )
                    return
                ...
                await message.answer(
                    f"⚠️ Помилка отримання списку гаманців ({response.status_code}):\n<code>{error_detail}</code>",
                    parse_mode="HTML",
                )
...
async def tx_history_via_api(message: types.Message):
    ...
    if len(parts) < 2:
        await message.answer(
            "❗ Введіть адресу гаманця: <code>/tx &lt;адреса&gt; [ліміт]</code>\nНаприклад: <code>/tx EQ... 5</code>",
            parse_mode="HTML",
        )
        return
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if resp.status_code != 200:
                ...
                await message.answer(
                    f"⚠️ Помилка API ({resp.status_code}) при запиті історії:\n<code>{error_detail}</code>",
                    parse_mode="HTML",
                )
                return
    ...
    if not events_data:
        await message.answer(
            f"📭 Для гаманця <code>{address}</code> не знайдено транзакцій (за лімітом {limit}).", parse_mode="HTML"
        )
        return
...
async def wallet_summary_via_api(message: Message):
    ...
    if len(args) < 2:
        await message.answer(
            "❗ Будь ласка, вкажіть адресу гаманця: <code>/summary &lt;TON-адреса&gt;</code>", parse_mode="HTML"
        )
        return
    address = args[1].strip()
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if summary.get("total_tx_count") is not None:
                    text += f"<b>Всього транзакцій (збережено):</b> {summary['total_tx_count']}\n"
                if summary.get("first_activity_ts"):
                    text += f"<b>Перша активність:</b> {datetime.utcfromtimestamp(summary['first_activity_ts']).strftime('%Y-%m-%d %H:%M')}\n"
                if summary.get("last_activity_ts"):
                    text += f"<b>Остання активність:</b> {datetime.utcfromtimestamp(summary['last_activity_ts']).strftime('%Y-%m-%d %H:%M')}\n"
        ...
        try:
            ...
            # ... (обработка ошибок)
                ...
                try:
                    ...
                    error_detail = response.text[:200]
                await message.answer(
                    f"⚠️ Помилка пошуку ({response.status_code}):\n<code>{error_detail}</code>", parse_mode="HTML"
                )
        # ... (обработка httpx ошибок)
        except httpx.ConnectError as e:
...
async def export_history_command(message: Message):
    ...
    if len(args) < 2:
        await message.answer(
            "❗ Вкажіть адресу гаманця для експорту: <code>/export &lt;адреса&gt;</code>", parse_mode="HTML"
        )
        return
    address = args[1].strip()
        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🕸️ Відкрити граф у WebApp", web_app=WebAppInfo(url=f"{web_app_url}{graph_params}")
                )
            ]
        ]
    )
    await message.answer("Для перегляду графа зв'язків, будь ласка, відкрийте веб-додаток:", reply_markup=keyboard)
